	...
	<div id="register-page"class="page register-page">
		...
		<h2 class="register-page-title">
			<?php if ( ! get_option( 'users_can_register' ) && function_exists( 'bp_get_option' ) && (bool) bp_get_option( 'bp-enable-membership-requests' ) ) : ?>
				<?php esc_html_e( 'Request Membership', 'buddypress' ); ?>
			<?php else : ?>
				<?php esc_html_e( 'Create an Account', 'buddypress' ); ?>
			<?php endif; ?>
		</h2>
		...
		<?php bp_nouveau_template_notices(); ?>
			...
			<?php if ( 'request-details' === bp_get_current_signup_step() ) : ?>
				...
				<?php if ( bp_is_active( 'xprofile' ) && ( ( function_exists( 'bp_nouveau_has_signup_xprofile_fields' ) && bp_nouveau_has_signup_xprofile_fields( true ) ) || bp_nouveau_base_account_has_xprofile() ) ) : ?>

					<?php bp_nouveau_signup_hook( 'before', 'signup_profile' ); ?>

					<div class="register-section extended-profile<?php echo ( beehive()->options->get( 'key=hide-visibility-toggles&default=1' ) ) ? ' hide-toggles': ''; ?>" id="profile-details-section">
